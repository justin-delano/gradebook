...
Subjects and standards filter the grid.
...
Assignments is where the work itself is described: what it is called, which category it belongs to, which standards it assesses, and when it was given.
...
Trends draws the same information over time.

For one student it shows whether they are climbing or sliding, standard by standard.
